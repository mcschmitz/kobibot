...
encoder_input_data = np.zeros((len(questions), MAX_SEQ_LEN, len(questions_int_to_vocab)), dtype='float32')
decoder_input_data = np.zeros((len(answers), MAX_SEQ_LEN + 1, len(answers_int_to_vocab)), dtype='float32')
...
model_saver = ModelCheckpoint('models/CharacterBasedChatBot.h5', save_best_only=True)
model = character_based_lstm(num_encoder_tokens=len(questions_int_to_vocab),
